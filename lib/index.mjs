...
const DEFAULT_MEMORY_DIRECTORY = "~/.dsh/memory";
...
const PLUGIN_ID = "@zhang-guo-wen/dsh-memory";
...
function memorySource() {
	return {
		kind: "plugin",
		plugin: `${PLUGIN_ID}#${MEMORY_LOADER}`,
		form: "instructions"
	};
}
/**
...
function isMemorySource(source) {
	...
	const record = source;
	return record.kind === "plugin" && record.plugin === `@zhang-guo-wen/dsh-memory#memory-index`;
}
/**
...
let MemoryRemote = (() => {
	...
	return class MemoryRemote extends _classSuper {
		...
		*/
		...
		async store() {
			return new MemoryStore(await this.runtime.directoryFor(void 0), { maxFileBytes: this.runtime.maxFileBytes() });
		}
	};
})();
...
*/
function registerMemorySettings(ctx, config = {}) {
	const base = {
		enabled: config.enabled ?? true,
		directory: config.directory ?? "~/.dsh/memory"
	};
	let flags = () => ({ ...base });
