		...
		var MemorySectionController = class {
			...
			*/
			...
			saveDirectory() {
				...
				this.notice = null;
				this.publish();
				this.scope.set("directory", directory);
			}
			editIndex(content) {
		...
		*/
		async function apply(ctx) {
			...
			const controller = new MemorySectionController(ctx.settingsScope.bind({ namespace: MEMORY_SETTINGS_NS }), {
				...
				writeIndex: async (content) => {
					await unwrapRemote(() => namespace().writeIndex({ content }));
				},
				pick: async () => {
