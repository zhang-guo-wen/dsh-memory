	...
	factory: (require) => {
		...
		*/
		...
		const zh = {
			...
			"directory.hint": "每个项目一个独立子目录：{project} 按 Claude 的 projects 命名规则生成（同一仓库的 worktree 共用同一个），~ 表示用户目录。",
		...
		const en = {
			...
			"directory.hint": "One subdirectory per project: {project} is named the way Claude names its project directories (worktrees of one repository share it), and ~ is the user home.",
			"directory.placeholder": "~/.dsh/memory/{project}",
			"directory.save": "Save",
		...
		var MemorySectionController = class {
			...
			*/
			...
			setClaudeCompatible(value) {
				...
				this.notice = null;
				this.publish();
				this.scope.set("claudeCompatible", value);
			}
			saveDirectory() {
				if (!this.canWrite()) return;
